...
use super::{AlgFnOutputMsgbus, AlgInput, AlgOutput, Error, OutputValue, calculation::Calculation};
    ...
    TMsg: MsgDataBound,
{
    pub async fn spawn(mut self) -> Result<(), Error> {
        let mut calculation = Calculation::new(self.time_window);
        ...
        while let Some(input_value) = self.input.recv().await {
            ...
            self.output
                .send(ValueTime {
                    ...
                    time: output_value.timestamp,
                })
                .await
                .map_err(|_| Error::AlgTaskUnexpectedEnd(String::from("downsampling")))?;
        }

        Err(Error::AlgTaskUnexpectedEnd(String::from("downsampling")))
    }
}

...
use esp_idf_svc::nvs::{
    EspDefaultNvsPartition, EspKeyValueStorage, EspNvs, EspNvsPartition, NvsDefault,
};
    ...
    TStorageData: Debug + Default + DeserializeOwned + PartialEq + Serialize + 'static,
{
    ...
    let data = load_data(&mut storage)?;
    ...
    task_input(msgbus_input, msgbus_output, config, storage, data).await?;
    Ok(())
}
...
async fn task_input<TMsg, TStorageData>(
    ...
    config: Config<TMsg, TStorageData>,
    mut nvs: EspKeyValueStorage<NvsDefault>,
    data: TStorageData,
) -> Result<()>
where
    TMsg: MsgDataBound,
...
fn load_data<TStorageData>(storage: &mut EspKeyValueStorage<NvsDefault>) -> Result<TStorageData>
where
...
fn save_data<TStorageData>(
    nvs: &mut EspKeyValueStorage<NvsDefault>,
    data: &TStorageData,
) -> Result<()>
where
    TStorageData: Debug + Default + DeserializeOwned + Serialize,

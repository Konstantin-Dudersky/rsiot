...
///
/// На рисунке приведена структура работы драйвера устройства.
/// В драйвере реализуется логическая часть опроса устройств. Физическая реализация взамодействия с
/// интерфейсом реализована в конкретных компонентах. Таким образом, драйвера можно использовать в
/// разных реализациях. Например, драйвера устройств I2С можно использовать как в компоненте
/// cmp_linux_i2c_master, так и cmp_esp_i2c_master.
///
#[doc = include_str!("./doc/diagram.svg")]
///
/// Цифрами 1, 2, 3, 4 помечены каналы передачи данных, соответствующие каналам на структуре
/// компонента опроса устройств. В таблице приведено описание задач.
///
/// | Название | Описание |
/// | ------- | ------- |

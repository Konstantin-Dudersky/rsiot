...
impl<'d> WS2812RMT<'d> {
    ...
    pub fn set_pixel(&mut self, rgb: RGB8) -> Result<()> {
        ...
        let t0h = Pulse::new_with_duration(ticks_hz, PinState::High, &ns(350))?;
        let t0l = Pulse::new_with_duration(ticks_hz, PinState::Low, &ns(900))?;
        let t1h = Pulse::new_with_duration(ticks_hz, PinState::High, &ns(900))?;
        let t1l = Pulse::new_with_duration(ticks_hz, PinState::Low, &ns(350))?;
